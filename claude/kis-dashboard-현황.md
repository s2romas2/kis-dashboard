...
## 📑 리포트 수집 소스 확장 (9/16 새벽 무인 예약작업 kis-reports-brokers — 커밋 8a9c241 reports.html · b49a8a7/c9acda10/5bfce6ff reports.py · f9b81094 문서 · 결과 da3f47b5·2ed68f9e·164982d9)
...
- 실행 시간: 수집 ~2분 + PDF 측정 200건 ~10분 + 해외 번역 → 13분(timeout 30분). **run #49(c9acda10) 성공 8분 15초**: 수집 237초·PDF 측정 82건·총 495초, 산업 184·기업 177(하나증권 홈페이지가 미국 러너에서 20초 타임아웃 → 0건, 네이버 경유 32건으로 커버 → 5bfce6ff에서 페이지 요청 1회 재시도(20→40초) 추가 → **run #50 성공 5분 25초, 하나증권 13+19건 복구, 산업 160·기업 169**(7일 창이 9/9~로 이동, 결과 164982d9)). c9acda10에서 DEBUG 즉시 출력(Actions 로그에서 진행 위치 확인 가능)·PDF 측정 시간예산 480초·병합 직후 reports.json 중간 저장(타임아웃돼도 목록 보존, 커밋은 if:always)·PDF 타임아웃 60→25초로 보강. 미측정 페이지수는 다음 실행(평일 09:00·12:00 KST)에서 채움
...
## 미해결
- 리포트 수집(9/16): 평일 09:00/12:00 정기실행 시간(5~13분) 관찰 / 신영·DS·iM·LS 직접 수집 탐사 / KB PDF는 로그인 필요라 요약만 / 대신 잘린 제목 보완(상세 페이지 rowid로 원제목)
