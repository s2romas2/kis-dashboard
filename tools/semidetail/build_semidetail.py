...
def scrub(s):
    if not isinstance(s, str):
        return s
    # 목표주가/TP + 금액이 함께 있는 문장(마침표 단위)은 통째로 제거 (예: '목표주가 디아이 39,000원 vs …', '3.3만원 → 2.4만원')
    s = re.sub(r'[^.]*(?:목표주가|목표가|적정주가|\bTP\b)[^.]*?\d[\d,.]*\s*(?:만\s*)?원[^.]*\.?', '', s)
...
FIELD_META = {  # 세부분야 id, 상위 그룹, 공정코드
    ...
    'STF': ('stf', '테스트부품', ['t']), '프로브카드 세라믹 STF': ('stf', '테스트부품', ['t']),
    '세라믹 STF': ('stf', '테스트부품', ['t']), '프로브카드': ('probe', '테스트부품', ['t']),
